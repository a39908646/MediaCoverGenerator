...
from mediacovergenerator.style.style_static_2 import (
    add_film_grain,
    align_image_right,
    darken_color,
    find_dominant_vibrant_colors,
)
from mediacovergenerator.utils.color_helper import ColorHelper
from mediacovergenerator.utils.ffmpeg import (
    animation_output_suffix,
    build_animation_export_command,
    run_ffmpeg,
)
...
def create_style_animated_2(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            output_ext = animation_output_suffix(animation_format)
            output_file = tmp_path / f"output{output_ext}"
            ffmpeg_cmd = build_animation_export_command(
                tmp_path / "frame_%04d.bmp",
                safe_fps,
                animation_format,
                output_file,
            ...
            if not run_ffmpeg(
                ...
                timeout_seconds=max(120, total_frames * 2),
                label=f"动态风格 2 {str(animation_format).upper()} 导出",
            ):
                return False
